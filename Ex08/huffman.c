...
int main() {
    ...
    for(i = 0; S[i] != '\0'; i++) {
       index[ (int)S[i] ]++;         /* [ Complete Here!! ] */ // Count the appearance of each character
    }
    ...
    construct_2(
        data,
        i); //出現数を持った文字たちをヒープに入れる（「親は子より出現頻度の値が小さい」という順序関係に基づいてヒープ化する＝
            // minimum heap）

    while(i > 1) {

        /* Pick up the 2 minimum appearance characters */
        couple[0] = delete_min(data, &i); /* [ Complete Here!! (Write function call) ] */; //ヒープから？？？
        couple[1] = delete_min(data, &i); /* [ Complete Here!! (Write function call) ] */; //ヒープから？？？
        ...
        newnode.count = couple[0].count + couple[1].count;     /* [ Complete Here!! ] */; // 取り出した2つの節点の値（出現回数）の合計
        ...
        head[num_of_newroot]->c_left = left;    /* [ Complete Here!! ] */ //作成した左側用ノードを木の左側に接続する
        head[num_of_newroot]->c_right = right;  /* [ Complete Here!! ] */ //作成した右側用ノードを木の右側に接続する

        num_of_newroot++;
        insert(data, newnode, &i); /* [ Complete Here!! (Write function call) ] */ //作成したノードをヒープに戻す
    ...
    printf("Compressibility  %.3f \n", (double)result / ( (n - 1) * BITS_PER_CHAR ) );
...
void insert(Node *A, Node v, int *i) {
    ...
    A[n] = v;
    upheap(A, n);  /* [ Complete Here!! (Write function call) ] */ //ヒープに戻す（Ex02とほぼ同じ）
}
...
int Huffman_DFS(NodePointer p, char *bin) {
    ...
    while(1) {
        ...
        if( p->c_left != NULL && p->c_left->vertex.visited == 0 ) { // DFSの要領で二分木を左側優先でたどる（もし、左側の子が探索可能で、かつ未到達であるならば。
                                        // Ex03とほぼ同じ）
            strcat(bin, "1");
            i++;
            totalbits = Huffman_DFS(p->c_left, bin);
        } else if(
            p->c_right != NULL && p->c_right->vertex.visited == 0 ) { // DFSの要領で二分木を右側にたどる（もし、右側の子が探索可能で、かつ未到達であるならば。
                                         // Ex03とほぼ同じ）
            strcat(bin, "0");
            i++;
