...
#define RND 10
#define BASE 64 // Strassenの性能を出すためには、この値の調整が重要
...
int main(int argc, char *argv[]) {
    ...
    printf("n3_product  %dx%d     : elapsed time is %10.6f sec. \n", mat_size, mat_size,
...
void strassen(int **A, int **B, int **C, int size) {
    ...
    X = (int **)malloc(/* [ Complete Here!! ] */ sizeof(int *) * mid );
    Y = (int **)malloc(/* [ Complete Here!! ] */ sizeof(int *) * mid );
    P1 = (int **)malloc(/* [ Complete Here!! ] */sizeof(int *) * mid );
    P2 = (int **)malloc(/* [ Complete Here!! ] */sizeof(int *) * mid );
    P3 = (int **)malloc(/* [ Complete Here!! ] */sizeof(int *) * mid );
    P4 = (int **)malloc(/* [ Complete Here!! ] */sizeof(int *) * mid );
    P5 = (int **)malloc(/* [ Complete Here!! ] */sizeof(int *) * mid );
    P6 = (int **)malloc(/* [ Complete Here!! ] */sizeof(int *) * mid );
    P7 = (int **)malloc(/* [ Complete Here!! ] */sizeof(int *) * mid );
    for(i = 0; i < mid; i++) {
        X[i] = (int *)malloc(/* [ Complete Here!! ] */  sizeof(int) * mid );
        Y[i] = (int *)malloc(/* [ Complete Here!! ] */  sizeof(int) * mid );
        P1[i] = (int *)malloc(/* [ Complete Here!! ] */  sizeof(int) * mid );
        P2[i] = (int *)malloc(/* [ Complete Here!! ] */  sizeof(int) * mid );
        P3[i] = (int *)malloc(/* [ Complete Here!! ] */  sizeof(int) * mid );
        P4[i] = (int *)malloc(/* [ Complete Here!! ] */  sizeof(int) * mid );
        P5[i] = (int *)malloc(/* [ Complete Here!! ] */  sizeof(int) * mid );
        P6[i] = (int *)malloc(/* [ Complete Here!! ] */  sizeof(int) * mid );
        P7[i] = (int *)malloc(/* [ Complete Here!! ] */  sizeof(int) * mid );
    }
    ...
    for(i = 0; i < mid; i++) {
        for(j = 0; j < mid; j++) {
            X[i][j] = A[i][j + mid] -
                      A[mid + i][mid + j]; // mid は行列を４分割する前の中央線に相当する。これをうまく使いこなすこと
    ...
    */
    ...
    for(i = 0; i < mid; i++) {
        for(j = 0; j < mid; j++) { // つまり P1 から P7 を使って C を求める
            ...
            */
            ...
            C[i][j + mid] = P4[i][j] + P5[i][j];
            C[i + mid][j] = P6[i][j] + P7[i][j];
            C[i + mid][j + mid] = P2[i][j] - P3[i][j] + P5[i][j] - P7[i][j];
        }
    }
...
void n3_product(int **A, int **B, int **C, int n) { //線形代数でおなじみの、普通の行列積の解き方
    int i, j, k;
